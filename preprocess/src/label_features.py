...
# @jit(nopython=True)
def calc_force_entry_price(entry_price: np.ndarray, boundary: np.ndarray, pips: float = 1.0, position: str = "buy") -> Tuple[np.ndarray, np.ndarray]:
    if pips <= 0:
        raise ValueError(f"Invalid 'pips' valuse {pips}. 'pips' shoud be more than 0")
    if position not in ["buy", "sell"]:
        raise ValueError(f"Invalid position 'value' {position}. position should be 'buy' or 'sell'")
...
def position_force_entry_price(df: pd.DataFrame, pips: float = 1.0, position: str = "buy") -> Tuple[np.ndarray, np.ndarray]:
    ...
    if position == "buy":
        ...
        buy_force_entry_price, buy_force_entry_time = calc_force_entry_price(buy_price.values, df["Low"].values, pips, "buy")
        return buy_force_entry_price, buy_force_entry_time
    else:
        sell_price = position_price_by_atr(df, pips, "sell")
        sell_force_entry_price, sell_force_entry_time = calc_force_entry_price(sell_price.values, df["High"].values, pips, "sell")
        return sell_force_entry_price * (-1), sell_force_entry_time
